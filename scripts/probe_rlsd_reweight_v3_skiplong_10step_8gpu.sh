...
# ============================================================
# RLSD teacher_reweight debug v3 distribution probe
# 目的：
#   1. 跑 10 step，观察更稳定的 reweight 分布
#   2. 使用 8 张卡
#   3. 不上传 W&B
#   4. 基本不保存 checkpoint
#   5. 启用 long-response gate
#   6. dump 300 条 token-level reweight debug
# ============================================================
...
# ---------- W&B ----------
# debug distribution run 不上传 wandb，避免污染正式曲线
...
DUMP_DIR="/data/xhou/debug_dumps/rlsd_token_reweight_probe_v3_skiplong_10step"
DUMP_FILE="${DUMP_DIR}/reweight.jsonl"

RAY_HEAD_PORT="${RAY_HEAD_PORT:-6401}"
RAY_DASHBOARD_PORT="${RAY_DASHBOARD_PORT:-8281}"
...
python -m verl.trainer.main \
  ...
  trainer.experiment_name=rlsd_token_reweight_probe_v3_skiplong_10step_8gpu \
  trainer.save_checkpoint_path=/data/xhou/checkpoints/rlsd_token_reweight_probe_v3_skiplong_10step_8gpu \
  ...
  algorithm.teacher_reweight_dump_max_samples=300 \
  algorithm.teacher_reweight_dump_max_correct=50 \
  algorithm.teacher_reweight_dump_max_incorrect=250
...
python scripts/summarize_reweight.py "${DUMP_FILE}" --topk 50

# ---------- Extra inspection ----------
echo "[INFO] Inspect skipped long-response samples:"
python - <<'PY'
import json
from collections import Counter

path = "/data/xhou/debug_dumps/rlsd_token_reweight_probe_v3_skiplong_10step/reweight.jsonl"
...
print(f"[INFO] Loaded records: {len(rows)}")

reason_counter = Counter(r.get("inactive_reason", "<missing>") for r in rows)
print("[INFO] inactive_reason counts:")
for k, v in reason_counter.most_common():
    print(f"  {k}: {v}")

active = [r for r in rows if r.get("inactive_reason") == "<active>" or r.get("teacher_reweight_active")]
skipped = [r for r in rows if r.get("skip_reweight_due_to_long_response")]
hitmax = [r for r in rows if r.get("hit_max_response_len")]

print(f"[INFO] active samples: {len(active)}")
print(f"[INFO] skipped long-response samples: {len(skipped)}")
print(f"[INFO] hit max-response samples: {len(hitmax)}")

if skipped:
    print("\n[INFO] Long-response skipped sample summary:")
    for r in skipped[:20]:
        print("-" * 100)
        print("sample_idx:", r.get("sample_idx"))
        print("global_step:", r.get("global_step"))
        ...
        print("prompt:", str(r.get("prompt", ""))[:250].replace("\n", " "))

if active:
    print("\n[INFO] Active sample summary:")
    for r in active[:20]:
        print("-" * 100)
        print("sample_idx:", r.get("sample_idx"))
        print("global_step:", r.get("global_step"))
        ...
        print("ground_truth_answer:", r.get("ground_truth_answer"))
        print("prompt:", str(r.get("prompt", ""))[:250].replace("\n", " "))
        print("response:", str(r.get("response_text", ""))[:500].replace("\n", " "))
PY