...
export WANDB_NAME="qwen3vl8b_sdpo_student_topk_tail_smoke_2gpu"
export WANDB_MODE="offline"

# ---------- SDPO debug dump ----------
export SDPO_DEBUG_DUMP=1
export SDPO_DEBUG_DUMP_PATH="/ssd5/xhou/outputs/sdpo_debug_student_topk_tail_smoke_2gpu.jsonl"
export SDPO_DEBUG_MAX_SAMPLES=10

# ---------- Skip-group debug ----------
export SDPO_SKIP_DEBUG=1
export SDPO_SKIP_DEBUG_MAX_GROUPS=2

# ---------- Video sampling debug ----------
export SDPO_VIDEO_SAMPLING_DEBUG_MAX=4

# ---------- Keep only the most useful debug flags ----------
...
python -m verl.trainer.main \
  ...
  trainer.experiment_name=qwen3vl8b_sdpo_student_topk_tail_smoke_2gpu \
  trainer.n_gpus_per_node=2 \
  algorithm.loss_mode=sdpo_logit \
  algorithm.sdpo_approx_mode=student_topk_tail \
  algorithm.sdpo_topk=32 \
  algorithm.sdpo_divergence=forward_kl \
  algorithm.sdpo_use_tail=false \
