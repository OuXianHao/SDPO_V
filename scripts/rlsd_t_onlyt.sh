...
# RLSD-only ablation: no visual_cf, no sdpo_v aux loss
EXP_NAME="qwen3vl8b_newdata_frames32_rlsd_only_lam05_decay70_run1"
...
python -m verl.trainer.main \
  ...
  algorithm.visual_cf_enabled=false \
  algorithm.visual_cf_use_keyframe_mask=false \
  algorithm.visual_cf_bad_video_mode=reuse_existing \
  algorithm.visual_cf_visual_weight=0.0 \
